...
export default function Admin({ user, onLogout }) {
  ...
  const [bulkPricingFilter, setBulkPricingFilter] = useState('All'); // 'All' | 'Sandstone' | 'Porcelain' | 'Bricks'
  const [bulkAdjustmentType, setBulkAdjustmentType] = useState('percentage'); // 'percentage' | 'fixed' | 'flat'
  const [bulkAdjustmentValue, setBulkAdjustmentValue] = useState('');
  const [bulkAdjustmentDirection, setBulkAdjustmentDirection] = useState('increase'); // 'increase' | 'decrease'
  ...
  useEffect(() => {
    if (products && products.length > 0) {
      ...
      setPricingPrices(initialPrices);
    }
  }, [products]);

  const handlePriceChange = (id, val) => {
    setPricingPrices(prev => ({ ...prev, [id]: val }));
  };
  ...
  const applyBulkAdjustment = () => {
    ...
    setPricingPrices(prev => {
      const updated = { ...prev };
      products.forEach(p => {
        if (bulkPricingFilter !== 'All' && p.category !== bulkPricingFilter) {
          return;
        }

        const currentPrice = Number(updated[p.id] !== undefined ? updated[p.id] : p.price);
        let newPrice = currentPrice;

        if (bulkAdjustmentType === 'flat') {
          newPrice = val;
        } else if (bulkAdjustmentType === 'percentage') {
          const factor = val / 100;
          if (bulkAdjustmentDirection === 'increase') {
        ...
        updated[p.id] = parseFloat(newPrice.toFixed(2));
      });
      return updated;
    });

    showToast('success', `Bulk adjustment calculated locally. Click 'Save All Changes' to save permanently.`);
  };
  ...
  const handleSaveAllPrices = async () => {
    ...
    try {
      const changedProducts = products.filter(p => {
        const localPrice = pricingPrices[p.id];
        return localPrice !== undefined && parseFloat(localPrice) !== parseFloat(p.price);
      });

      if (changedProducts.length === 0) {
        showToast('success', 'No pricing changes to save.');
        setLoading(false);
        return;
      }
      ...
      await Promise.all(
        changedProducts.map(p => {
          const updatedProduct = {
            ...p,
            price: parseFloat(parseFloat(pricingPrices[p.id]).toFixed(2))
          };
          return apiFetch(`/api/products/${p.id}`, {
            method: 'PUT',
      ...
      showToast('success', `Successfully updated pricing for ${changedProducts.length} products!`);
      await fetchData();
    } catch (err) {
      console.error('Failed to save pricing in bulk:', err);
      showToast('error', 'Failed to save pricing changes: ' + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const sortedProds = (prods || []).sort((a, b) => {
        const categoryOrder = { 'Sandstone': 1, 'Porcelain': 2, 'Bricks': 3 };
  ...
  // Converts a file to base64 and uploads via backend /api/upload-image (has service key — bypasses RLS)
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const imageUrl = await uploadImageViaBackend(file);
      if (editingProduct) {
      ...
      showToast('success', 'Primary image uploaded successfully!');
    } catch (err) {
  ...
  const handleSaveProduct = async (e) => {
    ...
    // Sanitize price — ensure it is stored as exact 2-decimal float, not a JS floating point artifact
    const rawPayload = isEdit ? editingProduct : newProductData;
    const payload = {
      ...rawPayload,
      price: parseFloat(parseFloat(rawPayload.price).toFixed(2)),
      stock: parseInt(rawPayload.stock) || 0,
      stars: parseFloat(rawPayload.stars) || 5
    };
    ...
    try {
      ...
      if (response.ok) {
        ...
        showToast('success', `Product "${payload.name}" saved successfully!`);
      } else {
        showToast('error', 'Save failed: ' + (data.message || 'Unknown error'));
      }
    } catch (err) {
      ...
      <div className="container">
        ...
        {toast && (
          <div style={{
            ...
            backgroundColor: toast.type === 'success' ? '#1a6633' : '#8B0000',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '240px 1fr', gap: '3.5rem' }} className="admin-layout">
          ...
          <aside style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
            ...
            <button 
              onClick={() => { setActiveTab('pricing'); setEditingProduct(null); setIsAddingProduct(false); }}
              style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '1.25rem 1rem', border: '1px solid var(--color-border-light)', backgroundColor: activeTab === 'pricing' ? '#FFFFFF' : 'transparent', color: activeTab === 'pricing' ? 'var(--color-accent)' : 'var(--text-muted-on-light)', fontWeight: activeTab === 'pricing' ? 600 : 500, cursor: 'pointer', textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.8rem' }}
            >
              <Coins size={16} /> Pricing Manager
            </button>
            <button 
          ...
          <div style={{ border: '1px solid var(--color-border-light)', padding: '3rem', backgroundColor: 'transparent' }}>
            ...
            {loading ? (
              ...
              <>
                      ...
                      <form onSubmit={handleSaveProduct} style={{ display: 'flex', flexDirection: 'column', gap: '1.75rem' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }} className="form-grid-2">
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                            ...
                            <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                              ...
                              />
                              <label htmlFor="admin-file-upload" className="btn btn-secondary" style={{ display: 'inline-flex', gap: '0.5rem', height: '48px', padding: '0 1.5rem', cursor: uploadingImage ? 'not-allowed' : 'pointer', alignItems: 'center', fontSize: '0.75rem', opacity: uploadingImage ? 0.6 : 1 }}>
                                <Upload size={14} /> {uploadingImage ? 'Uploading...' : 'Choose File'}
                              </label>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', borderTop: '1px solid var(--color-border-light)', paddingTop: '1.5rem', marginTop: '0.5rem' }}>
                          ...
                          <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                            ...
                            />
                            <label htmlFor="admin-gallery-file-upload" className="btn btn-secondary" style={{ display: 'inline-flex', gap: '0.5rem', height: '48px', padding: '0 1.5rem', cursor: uploadingImage ? 'not-allowed' : 'pointer', alignItems: 'center', fontSize: '0.75rem', opacity: uploadingImage ? 0.6 : 1 }}>
                              <Upload size={14} /> {uploadingImage ? 'Uploading...' : 'Upload & Add to Gallery'}
                            </label>
                          </div>
                        </div>
                ...
                {activeTab === 'pricing' && (
                  <div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2.5rem', borderBottom: '1px solid var(--color-border-light)', paddingBottom: '1rem' }}>
                      <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '1.8rem', fontWeight: 400 }}>Pricing Manager</h2>
                      <button 
                        onClick={handleSaveAllPrices} 
                        className="btn btn-primary"
                        style={{ height: '44px', padding: '0 2rem', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}
                      >
                        Save All Changes
                      </button>
                    </div>
                    ...
                    <div style={{ border: '1px solid var(--color-border-light)', padding: '2rem', marginBottom: '3rem', backgroundColor: '#FAFAF8' }}>
                      <h3 style={{ fontSize: '1.05rem', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1.5rem', color: 'var(--text-on-light)', fontWeight: 650 }}>Bulk Adjust Prices</h3>
                      <div style={{ display: 'flex', gap: '1.25rem', alignItems: 'center', flexWrap: 'wrap' }}>
                        
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', minWidth: '150px' }}>
                          <label style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-muted-on-light)' }}>Category</label>
                          <select 
                            value={bulkPricingFilter}
                            onChange={(e) => setBulkPricingFilter(e.target.value)}
                            style={{ padding: '0.75rem', border: '1px solid var(--color-border-light)', backgroundColor: '#FFFFFF', fontSize: '0.85rem' }}
                          >
                            <option value="All">All Categories</option>
                            <option value="Sandstone">Sandstones Only</option>
                            <option value="Porcelain">Porcelains Only</option>
                            <option value="Bricks">Bricks Only</option>
                          </select>
                        </div>

                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', minWidth: '140px' }}>
                          <label style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-muted-on-light)' }}>Adjustment</label>
                          <select 
                        ...
                        {bulkAdjustmentType !== 'flat' && (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', minWidth: '110px' }}>
                            <label style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-muted-on-light)' }}>Direction</label>
                            <select 
                              value={bulkAdjustmentDirection}
                              onChange={(e) => setBulkAdjustmentDirection(e.target.value)}
                              style={{ padding: '0.75rem', border: '1px solid var(--color-border-light)', backgroundColor: '#FFFFFF', fontSize: '0.85rem' }}
                            >
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', width: '120px' }}>
                          <label style={{ fontSize: '0.72rem', fontWeight: 600, color: 'var(--text-muted-on-light)' }}>Value</label>
                          <input 
                            type="number" 
                            step="any"
                            placeholder={bulkAdjustmentType === 'percentage' ? '10' : '50'}
                        ...
                        <button 
                          ...
                          className="btn btn-secondary"
                          style={{ alignSelf: 'flex-end', height: '46px', padding: '0 1.5rem', fontSize: '0.75rem' }}
                        >
                          Preview Adjustment
                        </button>
                      </div>
                    ...
                    <div style={{ overflowX: 'auto' }}>
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid var(--color-border-light)', textAlign: 'left', backgroundColor: '#FAFAF8' }}>
                            <th style={{ padding: '1rem', color: 'var(--text-muted-on-light)', fontWeight: 600 }}>Product Name</th>
                            <th style={{ padding: '1rem', color: 'var(--text-muted-on-light)', fontWeight: 600 }}>Category</th>
                            <th style={{ padding: '1rem', color: 'var(--text-muted-on-light)', fontWeight: 600 }}>Original Price</th>
                            <th style={{ padding: '1rem', color: 'var(--text-muted-on-light)', fontWeight: 600, width: '200px' }}>New Price (£)</th>
                            <th style={{ padding: '1rem', color: 'var(--text-muted-on-light)', fontWeight: 600 }}>Status</th>
                          </tr>
                        </thead>
                        <tbody>
                          {products.map(p => {
                            const localPrice = pricingPrices[p.id] !== undefined ? pricingPrices[p.id] : p.price;
                            const isChanged = parseFloat(localPrice) !== parseFloat(p.price);
                            return (
                              <tr key={p.id} style={{ borderBottom: '1px solid var(--color-border-light)', backgroundColor: isChanged ? '#FDF9F2' : 'transparent' }}>
                                <td style={{ padding: '1rem', fontWeight: 500 }}>{p.name}</td>
                                <td style={{ padding: '1rem', color: 'var(--text-muted-on-light)' }}>{p.category}</td>
                                <td style={{ padding: '1rem', color: 'var(--text-muted-on-light)' }}>£{Number(p.price).toFixed(2)}</td>
                                <td style={{ padding: '1rem' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                                    ...
                                    <input 
                                      ...
                                      style={{ padding: '0.4rem 0.6rem', border: '1px solid var(--color-border-light)', width: '120px', fontSize: '0.85rem', fontWeight: isChanged ? 600 : 400, color: isChanged ? 'var(--color-accent)' : 'var(--text-on-light)' }}
                                    />
                                  </div>
                                </td>
                                <td style={{ padding: '1rem' }}>
                                  {isChanged ? (
                                    <span style={{ color: 'var(--color-accent)', fontWeight: 600, fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Pending Save</span>
                                  ) : (
                                    <span style={{ color: 'var(--text-muted-on-light)', fontSize: '0.72rem' }}>Unchanged</span>
                                  )}
                                </td>
                              </tr>
                            );
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: '2.5rem' }}>
                      <button 
                        onClick={handleSaveAllPrices} 
                        className="btn btn-primary"
                        style={{ height: '48px', padding: '0 2.5rem', fontSize: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 700 }}
                      >
                        Save All Changes
                      </button>
                    </div>
