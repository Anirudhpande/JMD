...
const CATEGORY_TABS = [
  ...
  { label: 'Porcelain', slug: 'porcelain', img: '/images/porcelain_category.png' },
...
const HERO_SLIDES = [
  {
    ...
    price:    '£292.00',
    img:      '/images/raj-green-18-9m2-project-packs/primary.png',
  ...
  },
  {
    ...
    price:    '£373.00',
    img:      '/images/county-anthracite-900x600x20mm/primary.png',
    slug:     'county-anthracite-porcelain-900x600x20mm',
    accent:   '#4A5568',
  ...
  },
  {
    ...
    price:    '£292.00',
    img:      '/images/rippon-buff-18-9m2-project-packs/Primary.png',
...
const reviews = [
  ...
  { name: 'Sarah M. — Southampton Yard',  rating: 5, comment: 'Extremely premium look! The porcelain is perfect on our patio.' },
  { name: 'James K. — Chester',      rating: 5, comment: 'Great customer service from Roopesh. Consistent thickness.' },
];

export default function Home({ addToCart }) {
  const [allProducts, setAllProducts]       = useState([]);
  const [activeCategory, setActiveCategory] = useState('sandstone');
  ...
  useSEO({
    title:       'Indian Sandstone & Porcelain Paving Direct',
    description: 'Import-direct Indian sandstone, porcelain paving & handmade bricks. Trade prices, nationwide UK kerbside delivery in wooden crates. Free samples available.',
  ...
  const matchingProduct = allProducts.find(p => p.slug === slide.slug);
  const slidePrice = matchingProduct ? `£${Number(matchingProduct.price || 0).toFixed(2)}` : slide.price;
  const slideImg = matchingProduct && matchingProduct.images?.[0] ? matchingProduct.images[0] : slide.img;
    ...
    <div style={{ backgroundColor: 'var(--bg-light)' }}>
      ...
      <section style={{ background: slide.bg, transition: 'background 0.8s ease', overflow: 'hidden', position: 'relative' }}>
        ...
        <div className="container hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', minHeight: '500px', alignItems: 'center', gap: '2rem', padding: '4rem 2.5rem' }}>
          ...
          {/* LEFT — text */}
          <div style={{ zIndex: 2 }} className="hero-text-container">
            ...
            <p style={{ fontSize: '1.12rem', color: '#222', lineHeight: 1.6, marginBottom: '2rem', maxWidth: '450px' }}>
              {slide.sub}
            </p>

            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', flexWrap: 'wrap' }}>
              <div>
                <div style={{ fontSize: '0.7rem', textTransform: 'uppercase', letterSpacing: '0.08em', color: '#555', marginBottom: '0.2rem', fontWeight: 600 }}>From</div>
                <div style={{ fontSize: '2.8rem', fontWeight: 850, color: '#111', fontFamily: 'var(--font-body)', letterSpacing: '-0.02em', lineHeight: 1 }}>{slidePrice}</div>
                <div style={{ fontSize: '0.7rem', color: '#777', fontWeight: 500, marginTop: '0.25rem' }}>ex. VAT / pack</div>
              </div>
      ...
      <section style={{ backgroundColor: '#fff', borderTop: '1px solid #E5E0D8', borderBottom: '1px solid #E5E0D8', padding: '1.5rem 0' }}>
        <div className="container" style={{ display: 'flex', justifyContent: 'space-around', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
          {[
            { icon: <Truck size={18} />,       label: 'Kerbside Delivery',   sub: 'Calculated by postcode' },
            { icon: <ShieldCheck size={18} />, label: 'Free Collection',          sub: 'from Southampton Yard'  },
      ...
      {/* ════════════════════════════════════════════════════════
          COMBINED COLLECTIONS & FEATURED MATERIALS
      ═══════════════════════════════════════════════════════ */}
      <section style={{ backgroundColor: 'var(--bg-light)', padding: '4.5rem 0 5.5rem' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 2.5fr', gap: '3rem' }} className="category-featured-grid">
            
            {/* Left side: Browse by Category */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
              <div>
                <span style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#C9A96E', fontWeight: 700 }}>Collections</span>
                <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '2rem', fontWeight: 400, marginTop: '0.25rem', marginBottom: '1rem', color: '#111' }}>Categories</h2>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem' }}>
                {CATEGORY_TABS.map(cat => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ width: '48px', height: '48px', overflow: 'hidden', backgroundColor: isActive ? '#222' : '#F5F0E8', border: '1px solid #ECE6DB', borderRadius: '4px', flexShrink: 0 }}>
                        <img src={cat.img} alt={cat.label} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                      </div>
                      <div>
                        <span style={{ fontSize: '0.95rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', color: isActive ? '#C9A96E' : '#111', display: 'block' }}>{cat.label}</span>
                        <span style={{ fontSize: '0.75rem', color: isActive ? '#aaa' : '#777', display: 'block', marginTop: '0.15rem' }}>
                          {cat.slug 
                            ? `${allProducts.filter(p => p.category.toLowerCase() === cat.slug).length} Products`
            ...
            {/* Right side: Featured Materials */}
            <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.5rem' }}>
                <div>
                  <span style={{ fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '0.12em', color: '#C9A96E', fontWeight: 700 }}>Curated Paving</span>
                  <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '2rem', fontWeight: 400, marginTop: '0.25rem', color: '#111', margin: '0.25rem 0 0' }}>Featured Collections</h2>
                </div>
                
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button 
                    onClick={() => scrollCarouselManual(-1)}
                    style={{ width: '38px', height: '38px', border: '1px solid #D9D2C5', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', backgroundColor: '#fff', transition: 'all 0.25s' }}
                  >
                    <ArrowLeft size={14} />
                  </button>
                  <button 
              ...
              {filteredProducts.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '4rem', color: '#aaa', backgroundColor: '#fff', border: '1px solid #EDE7DC', borderRadius: '4px', flexGrow: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                  <Package size={32} style={{ opacity: 0.3, marginBottom: '0.75rem' }} />
                  <p style={{ fontSize: '0.85rem', margin: 0 }}>No products in this category yet.</p>
                </div>
              ) : (
                <div 
                    ...
                    >
                      ...
                      <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.5rem', flexGrow: 1 }}>
                        ...
                        <Link to={`/products/${prod.slug}`}>
                          <h3 style={{ fontSize: '1.25rem', fontWeight: 600, color: '#111', lineHeight: 1.4, minHeight: '2.5rem' }}>{prod.name}</h3>
                        </Link>
                        ...
                        <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.4rem', marginTop: '0.25rem' }}>
                          <span style={{ fontSize: '1.2rem', fontWeight: 700, color: '#111', fontFamily: 'var(--font-heading)' }}>£{Number(prod.price || 0).toFixed(2)}</span>
