    ...
    const initialData = {
      ...
      reviews: [
        ...
        },
        {
          ...
          user_name: 'Sarah M. from Southampton Yard',
          rating: 5,
  ...
  async clearProducts() {
    if (pool) {
      try {
        ...
        console.log('✅ Cleared products table via PostgreSQL pool.');
      } catch (err) {
        console.warn('⚠️ PostgreSQL pool delete failed, falling back to Supabase API:', err.message);
      }
    }
    if (isSupabaseConfigured) {
      const { error } = await supabase.from('products').delete().neq('id', '');
      if (error) {
        console.error('❌ Supabase REST clear failed:', error);
      } else {
        console.log('✅ Cleared products table via Supabase REST API.');
      }
    }
  ...
  async saveProducts(productsList) {
    ...
    if (pool) {
      try {
        ...
        pgSuccess = true;
        console.log('✅ Saved products to Supabase PostgreSQL database.');
      } catch (err) {
        console.warn('⚠️ PostgreSQL save failed, falling back to Supabase REST API:', err.message);
      }
    }
    
    // If PG query failed or pool is not configured, write to Supabase REST client as fallback
    if (!pgSuccess && isSupabaseConfigured && supabase) {
      const { error } = await supabase.from('products').upsert(productsList, { onConflict: 'id' });
      if (error) {
        console.error('❌ Supabase REST upsert failed:', error);
      } else {
        console.log('✅ Saved products via Supabase REST API.');
        pgSuccess = true;
      }
    }
    
    // Always write to local JSON database as secondary fallback/local sync
    const store = readLocalDb();
  ...
  async updateProduct(id, updatedFields) {
    ...
    VALID_COLUMNS.forEach(col => {
      if (updatedFields[col] !== undefined) {
        // Round price to exactly 2 decimal places to prevent floating point inaccuracies
        if (col === 'price') {
          filteredFields[col] = Math.round((parseFloat(updatedFields[col]) || 0) * 100) / 100;
        } else {
    ...
    // 1. Update PostgreSQL if pool is configured
    if (pool) {
      try {
        ...
        if (keys.length > 0) {
          ...
          if (res.rows.length > 0) {
            updatedProduct = formatProduct(res.rows[0]);
          }
        } else {
          const res = await pool.query('SELECT * FROM products WHERE id = $1', [id]);
          if (res.rows.length > 0) updatedProduct = formatProduct(res.rows[0]);
        }
      } catch (err) {
        console.error('PostgreSQL updateProduct failed:', err.message);
      }
    }

    // 2. Update Supabase REST if configured
    if (isSupabaseConfigured && supabase) {
      try {
        const { data, error } = await supabase.from('products').update(filteredFields).eq('id', id).select().single();
        if (!error && data && !updatedProduct) {
          updatedProduct = formatProduct(data);
        }
      } catch (err) {
        console.error('Supabase updateProduct failed:', err.message);
      }
    }

    // 3. Always update local JSON database to keep local db.json in sync!
    const store = readLocalDb();
    ...
    if (index !== -1) {
      ...
      writeLocalDb(store);
      if (!updatedProduct) {
        updatedProduct = formatProduct(store.products[index]);
      }
    }
